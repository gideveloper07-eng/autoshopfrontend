  ...
  void _filterSearch(String query) {
    ...
    setState(() {
      _filteredRows = _rows.where((row) {
        return _cell(row, 'va_24').toLowerCase().contains(q) ||
            _cell(row, 'va_29').toLowerCase().contains(q) ||
            _cell(row, 'va_18').toLowerCase().contains(q) ||
            _cell(row, 'va_26').toLowerCase().contains(q);
      }).toList();
    });
  }
